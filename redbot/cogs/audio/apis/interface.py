        ...
        if not tracks or not getattr(playlist, "tracks", None):
            ...
            if not tracks:
                ...
                (results, called_api) = await self.fetch_track(
                    cast(commands.Context, ctx(player.channel.guild, player.channel.guild, self.cog)),
                    player,
        ...
        if tracks:
            ...
            while valid is False and multiple:
                ...
                if not await self.cog.is_query_allowed(
                    ...
                    if IS_DEBUG:
                        log.debug(
                            "Query is not allowed in %r (%d)", player.channel.guild.name, player.channel.guild.id
                        )
                    continue
                valid = True
            track.extras.update(
